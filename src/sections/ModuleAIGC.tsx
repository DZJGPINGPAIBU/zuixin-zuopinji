  ...
  return (
    <motion.div className="fixed inset-0 z-[90] bg-white overflow-y-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
      <div className="sticky top-0 z-50 flex items-center justify-between px-6 py-4 bg-white/80 backdrop-blur-md border-b border-black/5">
        <span className="text-xs font-mono text-black/30 tracking-wider">PROJECT 04</span>
      ...
      <div className="max-w-[900px] mx-auto px-6 pb-20">
        <motion.div className="py-10 text-center" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <h2 className="text-[clamp(28px,5vw,56px)] font-black tracking-tight">AI创意视觉</h2>
          <p className="font-mono text-xs text-black/25 mt-2 tracking-[0.2em] uppercase">AIGC Creative Visual Design</p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-10">
          {[
            ...
            <motion.div key={img.src} className="rounded-2xl overflow-hidden cursor-pointer border border-black/5" whileHover={{ scale: 1.02 }}
    ...
    <motion.div className="card-3d cursor-pointer group" whileHover={{ y: -6 }} transition={{ duration: 0.3 }} onClick={onClick}>
      <div className="card-3d-inner rounded-2xl overflow-hidden bg-white relative aspect-[4/5]">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-5">
          <span className="text-[10px] font-mono text-white/50 tracking-wider uppercase">PROJECT 04</span>
          <h3 className="text-white font-bold text-lg mt-1 leading-tight">AI创意视觉</h3>
          <p className="text-white/50 text-xs mt-1">AIGC全流程设计赋能</p>
        </div>
